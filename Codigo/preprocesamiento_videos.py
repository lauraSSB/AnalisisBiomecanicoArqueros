...
def visibilidad(landmarks, mp_pose):
    ...
    if all(v > 0.65 for v in visibilidad):
        return True
    else:
        return False
...
def derecha_atras(landmarks,mp_pose, camara): 
    if camara == "T":
        print("1",landmarks[mp_pose.PoseLandmark.RIGHT_FOOT_INDEX.value].z ," - ", landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].z 
            ,"2", landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].z ," - ", landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].z
                ,"3", landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value].y ," - ", landmarks[mp_pose.PoseLandmark.LEFT_HIP.value].y)
...
def lectura_auxiliar(path):
            ...
            if not pausa:  # Solo leer un nuevo frame si no está en pausa
                ...
                if not ret:
                    ...
                    if resultados.pose_landmarks:
                        ...
                        print(distancia," = ", landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value].y," - ",landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE.value].y)
                        print("D: ",distancia > mejor_distancia, " DA: ",derecha_atras(landmarks,mp_pose, camara), " V: ",visibilidad(landmarks, mp_pose))
    ...
    #     captura_final  = cv2.VideoCapture(path, cv2.CAP_FFMPEG)
    #     captura_final .set(cv2.CAP_PROP_POS_FRAMES, num_frame)
    #     with mp_pose.Pose(min_detection_confidence=0.85, min_tracking_confidence=0.90, model_complexity=2) as pose_final:
    #         for i in range(num_frame-2, frame_final + 1):
    #             print("for: ", i)
    #             ret, frame = captura_final.read()
    #             if ret:
    #                 frames_analizados += 1
    ...
    #                     hip_izq = get_coords(landmarks[mp_pose.PoseLandmark.LEFT_HIP])
    #                     knee_izq = get_coords(landmarks[mp_pose.PoseLandmark.LEFT_KNEE])
    #                     ankle_izq = get_coords(landmarks[mp_pose.PoseLandmark.LEFT_ANKLE])

    #                     # Coordenadas pierna derecha
    #                     hip_der = get_coords(landmarks[mp_pose.PoseLandmark.RIGHT_HIP])
    #                     knee_der = get_coords(landmarks[mp_pose.PoseLandmark.RIGHT_KNEE])
    #                     ankle_der = get_coords(landmarks[mp_pose.PoseLandmark.RIGHT_ANKLE])

    #                     # Dibujo pierna izquierda (azul)
    #                     cv2.line(frame, hip_izq, knee_izq, (255, 0, 0), 3)
    #                     cv2.line(frame, knee_izq, ankle_izq, (255, 0, 0), 3)

    #                     # Dibujo pierna derecha (verde)
    #                     cv2.line(frame, hip_der, knee_der, (0, 255, 0), 3)
    #                     cv2.line(frame, knee_der, ankle_der, (0, 255, 0), 3)
    #                     cv2.imshow("Mediapipe Pose", frame)
    ...
    #                     if camara == "T":
    #                         print(not derecha_atras(landmarks,mp_pose,camara))
    #                         #Para que sea el fin del video la derecha NO debe estar atras y debe cumplir con las coordenadas de fin
    #                         if fin_video_coordenadas(camara, landmarks, mp_pose, True) and not derecha_atras(landmarks,mp_pose,camara):
    #                             frame_corte_final = i
    #                             break
    #                     elif camara == "LI" or camara == "LD":
    #                         #Para que sea el fin del video la derecha NO debe estar atras y debe cumplir con las coordenadas de fin
    ...
    #                 # Mostrar el frame actualizado con las líneas
    #                 cv2.imshow("Mediapipe Pose", frame)
...
if __name__ == '__main__':
    ...
    #Corrección1
    valores = [81, 80, 79, 78, 77, 76, 75, 69, 67, 61, 44, 43, 42, 39, 37, 30, 25, 12, 11, 10, 5, 4, 3, 2, 1]
    #Corrección2
    ...
    for i in valores: 
        ...
        print(inicio_T,final_T)
        print(inicio_LI,final_LI)
        print(inicio_LD,final_LD)
