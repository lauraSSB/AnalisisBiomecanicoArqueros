...
imagen = cv2.imread("./Frames/frame1.jpg") 
...
output_path = os.path.join(carpeta_frame, "frame1_RGB_final.jpg")
cv2.imwrite(output_path, imagen_rgb)
imagen_hsv = cv2.cvtColor(imagen, cv2.COLOR_BGR2HSV)  
output_path = os.path.join(carpeta_frame, "frame1_HSV_final.jpg")
cv2.imwrite(output_path, imagen_hsv)
imagen_ycrcb = cv2.cvtColor(imagen, cv2.COLOR_BGR2YCrCb)
output_path = os.path.join(carpeta_frame, "frame1_YCrCb_final.jpg")
cv2.imwrite(output_path, imagen_ycrcb)
imagen_lab = cv2.cvtColor(imagen, cv2.COLOR_BGR2LAB)  
output_path = os.path.join(carpeta_frame, "frame1_LAB_final.jpg")
